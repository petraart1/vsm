...
class ScenarioSeedIntegrationTest {
     ...
     * загружены при старте. Намеренно {@code contains}, а не {@code containsExactlyInAnyOrder}:
     * набор seed-файлов расширяется, поэтому проверяется вхождение, а не точное совпадение.
     * Полное покрытие 51/51 проверяется отдельным подсчётом файлов, а не перечислением кодов здесь.
     */
    @Test
